...
module.exports = kconfig = async (kill, message) => {
    try {
		...
		case 'grupo':
            ...
            if (onar[0] == 'fechar') {
				kill.setGroupToAdminsOnly(groupId, true).then(() => kill.sendText(from, '⚠️ *Grupo fechado para evitar incidentes desagradáveis durante a madrugada , o grupo será reaberto pela manhã assim que o primeiro adm acordar* ⚠️ *Pedimos a compreensão de todos* ⚠️'))
        ...
        case 'mutepv':
            if (isOwner) {
				if (args[0] == 'on') {
					...
					slce.push(pvmt)
					fs.writeFileSync('./lib/config/silence.json', JSON.stringify(slce))
					await kill.reply(from, 'Ele não poderá usar a sofia.', id)
				} else if (args[0] == 'off') {
					if (args.length == 0) return kill.reply(from, 'Você deve definir [on e off] e em seguida o número da pessoa sem - ou +.', id)
					const pvmt = body.slice(11) + '@c.us'
					...
					slce.splice(pvtnm, 1)
					fs.writeFileSync('./lib/config/silence.json', JSON.stringify(slce))
					await kill.reply(from, 'Ele poderá usar a sofia novamente.', id)
				} else {
